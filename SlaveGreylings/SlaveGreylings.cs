...
namespace SlaveGreylings
{
    ...
    public class SlaveAI : MonsterAI
    {
        ...
        protected override void UpdateAI(float dt)
        {
            ...
            if (AvoidFire(dt, m_assignment.transform.position))
            {
                m_aiStatus = "Avoiding fire";
                return;
            }

            var UpdateConsumeItem_method = this.GetType().GetMethod("UpdateConsumeItem", BindingFlags.NonPublic | BindingFlags.Instance);
            ...
            if (m_assigned)
            {
                MoveTowards(m_assignment.transform.position, IsAlerted());
